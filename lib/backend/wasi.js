...
class WASI {
  ...
  init(options) {
    ...
    if (io.fs && options.preopens) {
      for (const alias of Object.keys(options.preopens)) {
        const path = options.preopens[alias];
  ...
  check(fd, rights) {
    ...
    if (rights !== 0 && !(stats.rights.base & rights))
  ...
  fd_allocate(fd, offset, len) {
    ...
    //   __wasi_filesize_t len
    // )
    this.check(fd, constants.WASI_RIGHT_FD_ALLOCATE);
    return constants.WASI_ENOSYS;
  }
  ...
  fd_filestat_get(fd, buf) {
    ...
    this.view.setUint32(buf + 24, Number(rstats.nlink), true);
  ...
  fd_filestat_set_size(fd, size) {
    ...
    io.fs.ftruncateSync(stats.handle, Number(size));
...
outer:
  ...
  fd_readdir(fd, buf, buf_len, cookie, bufused) {
    ...
    for (let i = Number(cookie); i < list.length; i++) {
      ...
      const full = io.path.resolve(stats.path, name);
  ...
  fd_renumber(fd, to) {
    ...
    //   __wasi_fd_t to
    // )
    const fstats = this.check(fd, 0);
    const tstats = this.check(to, 0);
  ...
  fd_seek(fd, offset, whence, newoffset) {
    ...
    if (pos < 0n || pos > 0xffffffffffffffffn)
      return constants.WASI_EINVAL;
  ...
  fd_tell(fd, offset) {
    ...
    this.view.setBigUint64(offset, stats.pos, true);
  ...
  path_create_directory(fd, path, path_len) {
    ...
    const p = this.buffer.toString('utf8', path, path + path_len);

    io.fs.mkdirSync(io.path.resolve(stats.path, p));
  ...
  path_filestat_get(fd, flags, path, path_len, buf) {
    ...
    const p = this.buffer.toString('utf8', path, path + path_len);
    const st = io.fs.statSync(io.path.resolve(stats.path, p));
    ...
    this.view.setUint32(buf + 24, Number(st.nlink), true);
  ...
  path_filestat_set_times(fd, flags, path, path_len, atim, mtim, fst_flags) {
    ...
    const mtime = mflag ? now : mtim;
    const p = this.buffer.toString('utf8', path, path + path_len);
    const full = io.path.resolve(stats.path, p);

    io.fs.utimesSync(full, millisec(atime), millisec(mtime));
  ...
  path_link(old_fd, old_flags, old_path, old_path_len,
    ...
    const op = this.buffer.toString('utf8', old_path, old_path + old_path_len);
    const np = this.buffer.toString('utf8', new_path, new_path + new_path_len);

    io.fs.linkSync(io.path.resolve(ostats.path, op),
                   io.path.resolve(nstats.path, np));
  ...
  path_open(fd, dirflags, path, path_len, oflags,
    ...
    let full = io.path.resolve(stats.path, p);
    let rel = io.path.relative(stats.path, full);

    if (rel.startsWith('..'))
      return constants.WASI_ENOTCAPABLE;

    try {
      full = io.fs.realpathSync(full);
      rel = io.path.relative(stats.path, full);
    ...
    const handle = io.fs.openSync(full, noflags);
  ...
  path_readlink(fd, path, path_len, buf, buf_len, bufused) {
    ...
    const p = this.buffer.toString('utf8', path, path + path_len);
    const full = io.path.resolve(stats.path, p);
    const r = io.fs.readlinkSync(full);
    const used = this.buffer.write(r, buf, buf_len, 'utf8');
  ...
  path_rename(fd, old_path, old_path_len, new_fd, new_path, new_path_len) {
    ...
    io.fs.renameSync(io.path.resolve(ostats.path, op),
                     io.path.resolve(nstats.path, np));
  ...
  path_symlink(old_path, old_path_len, fd, new_path, new_path_len) {
    ...
    io.fs.symlinkSync(op, io.path.resolve(stats.path, np));
  ...
  path_unlink_file(fd, path, path_len) {
    ...
    io.fs.unlinkSync(io.path.resolve(stats.path, p));
    ...
    for (let i = 0; i < nsubscriptions; i++) {
      ...
      switch (type) {
        case constants.WASI_EVENTTYPE_CLOCK: {
          ...
          this.view.setBigUint64(out + 16, 0n, true);
