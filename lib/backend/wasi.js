...
class WASI {
  constructor(options) {
    this._wasi = new JSWASI(new JSWASIOptions(options));
    this._started = false;
    this.wasiImport = this._wasi.toAPI();
  }
  ...
  start(instance) {
    ...
    if (!isMemory(exports.memory))
      throw new Error('Instance must export `memory`.');

    if (this._started)
      throw new Error('WASI already started.');

    this._started = true;
    this._wasi.setMemory(exports.memory);
    ...
    try {
      ...
      if (isUnreachableError(err))
        err = new WASIExitError(this._wasi.exitCode || 1);

      if (this._wasi.returnOnExit) {
...
class JSWASIOptions {
  constructor(options) {
    ...
    this.preopens = Object.create(null);
    this.returnOnExit = false;
    this.stdin = 0;
    this.stdout = 1;
    this.stderr = 2;
  ...
  init(options) {
    ...
    if (options.env !== undefined && !isObject(options.env))
      throw new TypeError('`options.env` must be an object.');

    if (options.preopens !== undefined && !isObject(options.preopens))
      throw new TypeError('`options.preopens` must be an object.');

    if (options.returnOnExit !== undefined
        && typeof options.returnOnExit !== 'boolean') {
      throw new TypeError('`options.returnOnExit` must be a boolean.');
    }

    if (options.stdin !== undefined
        && (options.stdin | 0) !== options.stdin) {
      throw new TypeError('`options.stdin` must be an integer.');
    }

    if (options.stdout !== undefined
        && (options.stdout | 0) !== options.stdout) {
      throw new TypeError('`options.stdout` must be an integer.');
    }

    if (options.stderr !== undefined
        && (options.stderr | 0) !== options.stderr) {
      throw new TypeError('`options.stderr` must be an integer.');
    }
    ...
    if (options.env) {
      for (const key of Object.keys(options.env)) {
        if (options.env[key] !== undefined)
          this.env[key] = String(options.env[key]);
      }
    }

    if (options.preopens) {
      for (const key of Object.keys(options.preopens))
        this.preopens[key] = String(options.preopens[key]);
    }

    if (options.returnOnExit !== undefined)
      this.returnOnExit = options.returnOnExit;
...
class JSWASI {
  ...
  init(options) {
    this.addSTDIO(constants.WASI_STDIN_FILENO, options.stdin, '<stdin>');
    this.addSTDIO(constants.WASI_STDOUT_FILENO, options.stdout, '<stdout>');
    this.addSTDIO(constants.WASI_STDERR_FILENO, options.stderr, '<stderr>');
...
function isInstance(instance) {
  return isObject(instance) && isObject(instance.exports);
}

function isMemory(memory) {
  if (!isObject(memory))
    return false;
  ...
  return typeof memory.grow === 'function';
}
