...
class WASM {
  ...
  updateViews() {
    ...
    this.b8 = Buffer.from(this.buffer, 0, this.buffer.length);
  ...
  reallocBuffer(size) {
    assert(this.params.ALLOW_MEMORY_GROWTH);

    const aligned = alignUp(size, WASM_PAGE_SIZE);
    const old = this.buffer.byteLength;

    let result = -1;

    try {
      result = this.memory.grow((aligned - old) / WASM_PAGE_SIZE);
    } catch (e) {
      ;
    }
  ...
  setBU64(ptr, value) {
    (new BigUint64Array(this.buffer))[ptr >>> 3] = value;
    return this;
  }

  setBI64(ptr, value) {
    (new BigInt64Array(this.buffer))[ptr >>> 3] = value;
    return this;
  }
  ...
  getBU64(ptr) {
    return (new BigUint64Array(this.buffer))[ptr >>> 3];
  }

  getBI64(ptr) {
    return (new BigInt64Array(this.buffer))[ptr >>> 3];
  }
  ...
  getU64Array(ptr, size) {
    ...
    if (ptr + size > (this.u32.length >>> 1))
      throw new RangeError('Out of bounds.');

    if (size !== 0 && ptr === 0)
      throw new RangeError('Invalid pointer.');

    return new BigUint64Array(this.buffer, ptr * 8, size);
  }
  ...
  getI64Array(ptr, size) {
    ...
    if (ptr + size > (this.i32.length >>> 1))
      throw new RangeError('Out of bounds.');

    if (size !== 0 && ptr === 0)
      throw new RangeError('Invalid pointer.');

    return new BigInt64Array(this.buffer, ptr * 8, size);
  }
  ...
  copy(ptr, value) {
    ...
    if (ptr + buf.length > this.u8.length)
      throw new RangeError('Out of bounds.');
